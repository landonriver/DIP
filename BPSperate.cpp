...
IMPLEMENT_DYNAMIC(BPSperate, CDialogEx)

BPSperate::BPSperate(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_BPSperate, pParent)
	, m_BitPlane(0)
{
...
void BPSperate::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(BPSperate, CDialogEx)
	ON_BN_CLICKED(IDC_BUT_Slicing, &BPSperate::OnBnClickedButSlicing)
	ON_WM_PAINT()
	ON_BN_CLICKED(IDC_BUTSaveBitPlane, &BPSperate::OnBnClickedButsavebitplane)
END_MESSAGE_MAP()
...
void BPSperate::OnBnClickedButSlicing()
{
	// 位平面分解 
...
void BPSperate::OnBnClickedButsavebitplane()
{	//保存处理后的位平面
	if (flag == 0) return;
